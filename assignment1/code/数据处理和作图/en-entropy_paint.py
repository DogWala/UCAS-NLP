...
corpus1 = open('v3-en-wiki-merged-cleaned.txt').read()
corpus2 = open('v3-en-news-merged-cleaned.txt').read()
corpus3 = open('v3-en-norvel-fulleng-cleaned.txt').read()
...
for entropy in [entropies1,entropies2,entropies3]:
    ...
    # 在最后一个点上添加文本注释
    x = last_x-3
    y1 = (last_y-0.0005) if (entropy != entropies3) else (last_y+0.0005)
    y2 = (last_y-0.004) if (entropy != entropies3) else (last_y+0.003)
    plt.annotate('{}'.format(last_y), 
                xy=(last_x, y1), 
                xytext=(x, y2), 
                arrowprops=dict(arrowstyle='->', connectionstyle='arc3,rad=0'))
# add the entropy curves to the plot
ax.plot(xarr, entropies1, label='wiki')
ax.plot(xarr, entropies2, label='news')
ax.plot(xarr, entropies3, label='norvel')
# add a legend and title
ax.legend(loc='lower right')
ax.set_title('Entropy of three English samples')
# x and y labels
plt.xlabel('Number of characters (M)')
plt.ylabel('Entropy')
# display the plot
plt.savefig('en-entropy' + str(num_points) + '*' + str(size) + '.png', dpi = 1000)